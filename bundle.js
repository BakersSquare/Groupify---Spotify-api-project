...
var spotifyApi = new SpotifyWebApi({
  clientId:'71d5e0f01594412c9ddd909810a4e69a',
  clientSecret:'d9da6362128a41c89fcfa1079844771d',
  redirectUri: 'http://127.0.0.1:5500/public/index.html'
})
...
//Spotify sample code. Executed with node
const APICode = (function() {

  const clientID = '71d5e0f01594412c9ddd909810a4e69a';
  const secret = 'd9da6362128a41c89fcfa1079844771d';
  const redirect_uri = 'http://127.0.0.1:5500/public/index.html'
  const authCode = "";
  const state = generateRandomString(16);

  const authToken = async () =>{    
  const queryReturn = await fetch('https://accounts.spotify.com/authorize?' + $.param({
    client_id: clientID,
    response_type: 'token',
  ...
  const data = await queryReturn.text();
  authCode = data;
  return data;
}
})();
//API code will fire immediately because it's assigned to a variable declaration. Assigning the variable upon running will invoke all the functions. Then we will have access to all of the information provided
...
async function test() {
 await fetch('https://accounts.spotify.com/authorize' + $.param({
  client_id: clientID,
  response_type: 'token',
  redirect_uri: redirect_uri,
  scope: 'playlist-modify-private playlist-modify-public playlist-read-private playlist-read-collaborative',
...
/*    Outline for Project
    playlist modify private = to create and items to a playlist 
    playlist modify public = to create and items to a playlist (both are necesary to make a new playlist that is of collaborative type)
    playlist read private = to get a list of users playlist. Get the ID's of playlists not owned by the user. This assumes a friendship
    playlist read collaborative = same idea as above, but includes collaborative playlists the user is a part of. Checks the owner to see if it's someone else. Assumes a friendship.
*/

/*
//The Coding Train 2.1
